...
export class BudgetTabComponent implements OnChanges, OnDestroy {
  ...
  private buildPdfDocument(logoDataUrl: string | null): jsPDF {
    ...
    // ── HEADER BANNER (official letterhead) ─────────────────────
    let y = drawLetterhead(doc, {
      title: 'PROJECT FINANCIAL REPORT',
      subtitle: `Generated on ${new Date(r.generatedAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })} at ${new Date(r.generatedAt).toLocaleTimeString()}`,
      logoDataUrl,
