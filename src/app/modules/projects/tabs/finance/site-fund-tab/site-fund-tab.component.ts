...
export class SiteFundTabComponent implements OnChanges, OnDestroy {
  ...
  constructor(
    ...
    // Fire-and-forget: by the time anyone clicks Print/Download, this is cached.
    loadCompanyLogo();
  }
  ...
  exportSummaryExcel(): void {
    ...
    const summarySheet = XLSX.utils.aoa_to_sheet([
      ...this.officialHeaderRows('SITE FUND SUMMARY REPORT', s),
      ['Item', 'Amount'],
      ['Money Received', +s.received.total],
      ['Materials — Subtotal', +s.materials.total],
      ['Labour — Subtotal', +s.labour.total],
      ['Site Fund Balance (Received − Materials − Labour)', this.siteFundBalance()],
      [],
      ['Payments — Subtotal (separate cash flow)', +s.payments.total],
    ]);
    summarySheet['!cols'] = [{ wch: 45 }, { wch: 18 }];
  ...
  async exportSummaryPdf(): Promise<void> {
    ...
    let y = drawLetterhead(doc, {
      title: 'SITE FUND SUMMARY REPORT',
      subtitle: `${s.project?.name || ''}  —  Period: ${this.periodLabel()}`,
      logoDataUrl: logo,
    });
  ...
  private buildPdfDocument(logoDataUrl: string | null): jsPDF {
    const doc = new jsPDF();
    const pageWidth = doc.internal.pageSize.getWidth();
    const marginX = 14;
    const contentWidth = pageWidth - marginX * 2;
    ...
    const period = this.filters.dateFrom || this.filters.dateTo
      ...
      : 'Period: All time';
    let y = drawLetterhead(doc, {
      title: 'SITE FUND REPORT',
      subtitle: `${sk.firstName} ${sk.lastName}  —  ${period}`,
      logoDataUrl,
  ...
  async printReport(): Promise<void> {
    ...
    const logo = await loadCompanyLogo();
    const doc = this.buildPdfDocument(logo);
    doc.autoPrint();
