...
export class SettingsComponent implements OnInit {
  ...
  ngOnInit() {
    this.clientSvc.getClients({ limit: 100 }).subscribe(res => {
      this.clientOptions = (res.data || []).map((c: any) => ({
        value: c.id, label: c.name, sublabel: c.company || '', color: '#1a56db'
      }));
    });
  ...
  loadStakeholderData() {
    ...
    this.http.get<any>(`${api}/projects/${this.projectId}/stakeholders`, { headers }).subscribe({
      next: res => { this.projectStakeholders = res.data?.stakeholders || []; }
    });
    ...
    this.http.get<any>(`${api}/stakeholder-types`, { headers }).subscribe({
      next: res => {
        ...
        this.typeOptions = this.stakeholderTypes.map((t: any) => ({
          value: t.id, label: t.name, color: t.color || '#6b7280'
        }));
      }
    });
  ...
  initStakeholderForm(ps?: any) {
    this.stakeholderForm = this.fb.group({
      stakeholderId:     [ps?.stakeholderId     || ps?.stakeholder?.id || '', Validators.required],
  ...
  onSubmitStakeholder() {
    if (this.stakeholderForm.invalid) { this.stakeholderForm.markAllAsTouched(); return; }
    ...
    if (this.editingPS) {
      this.http.put<any>(`${api}/projects/${this.projectId}/stakeholders/${this.editingPS.id}`, val, { headers }).subscribe({
        next: () => {
      ...
      this.http.post<any>(`${api}/projects/${this.projectId}/stakeholders`, val, { headers }).subscribe({
        next: () => {
          this.savingStakeholder = false;
          this.closeStakeholderModal();
          this.loadStakeholderData();
  ...
  removeStakeholder(ps: any) {
    Swal.fire({
      ...
      if (r.isConfirmed) {
        const api = environment.apiUrl;
        const headers = { Authorization: `Bearer ${localStorage.getItem('accessToken')}` };
        this.http.delete<any>(`${api}/projects/${this.projectId}/stakeholders/${ps.id}`, { headers }).subscribe({
          next: () => {
            this.loadStakeholderData();
            Swal.fire({ icon: 'success', title: 'Removed!', timer: 1200, showConfirmButton: false });
          },
          error: (err) => {
            Swal.fire({ icon: 'error', title: 'Error', text: err?.error?.message || 'Failed to remove.' });
          }
        });
      }
    });
  ...
  initForm(p: Project) {
    this.form = this.fb.group({
      ...
      totalBudget: [p.totalBudget || (p as any).budget || 0, [Validators.required, Validators.min(0)]],
  ...
  archiveProject() {
    Swal.fire({
      title: 'Archive Project?', icon: 'warning',
      showCancelButton: true, confirmButtonColor: '#f59e0b', confirmButtonText: 'Yes, archive!'
    }).then(r => {
      if (r.isConfirmed) {
        this.projectSvc.update(this.projectId, { status: 'on_hold' }).subscribe({
          next: () => Swal.fire({ icon: 'success', title: 'Archived!', timer: 1500, showConfirmButton: false })
        });
      }
    });
  }
  ...
  deleteProject() {
    Swal.fire({
      ...
      input: 'text', inputPlaceholder: `Type "${this.project?.name}" to confirm`,
      preConfirm: (val) => {
        if (val !== this.project?.name) { Swal.showValidationMessage('Name does not match!'); return false; }
        return true;
      }
    }).then(r => {
      if (r.isConfirmed) {
  ...
  get sf() { return this.stakeholderForm?.controls; }
}
